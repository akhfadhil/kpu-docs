<x-layout>
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Upload Documents C Hasil</h1>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                ...
                @foreach ($docs as $doc)
                    @php
                        ...
                        $existing = $tps->document->firstWhere('doc_type', $doc['name']) ?? null;
                    @endphp
                    <div
                        class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-4 flex flex-col items-center text-center hover:border-primary/50 transition">
                        ...
                        <button data-modal-target="modal-{{ Str::slug($doc['name']) }}"
                                ...
                                <form action="{{ route('upload.store') }}" method="POST" enctype="multipart/form-data"
                                    ...
                                    @if ($existing)
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            File sebelumnya: <span
                                                class="text-blue-600 dark:text-blue-400">{{ basename($existing->path) }}</span>
                                        </p>
                                    @endif
            ...
            <p class="text-center text-sm text-gray-400 dark:text-gray-500 mt-8">
                Supported file types: <span class="font-medium">2MB PDF</span>
            </p>
        </div>
    </div>
</x-layout>
